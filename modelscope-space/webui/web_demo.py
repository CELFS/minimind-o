import argparse, os, sys, json, time, math, torch, threading, queue, base64, io, logging, contextlib, wave, secrets
import numpy as np
import torchaudio
...
def _origin_token_from_request():
    token = request.headers.get('X-Origin-Service-Token', '').strip()
    authorization = request.headers.get('Authorization', '').strip()
    if not token and authorization.lower().startswith('bearer '):
        token = authorization[7:].strip()
    if not token:
        token = request.args.get('studio_token', '').strip()
    return token

@app.before_request
def require_origin_service_token():
    if request.path == '/health':
        return None
    if not ORIGIN_SERVICE_TOKEN:
        if REQUIRE_ORIGIN_AUTH:
            return Response(json.dumps({'ok': False, 'error': 'origin authentication is not configured'}), status=503, mimetype='application/json')
        return None
    token = _origin_token_from_request()
    if not token or not secrets.compare_digest(token, ORIGIN_SERVICE_TOKEN):
        return Response(json.dumps({'ok': False, 'error': 'unauthorized'}), status=401, mimetype='application/json')
    return None
...
def load_main_model(model_path, model_name):
    with MODEL_LOCK:
        ...
        vision_encoder, vision_processor = MiniMindOmni.load_vision(os.path.join(MODEL_DIR, 'siglip2-base-p32-256-ve'))
        audio_encoder, audio_processor = MiniMindOmni.load_sensevoice(os.path.join(MODEL_DIR, 'SenseVoiceSmall'))
...
@app.route('/health')
def health():
    return Response(json.dumps({'ok': True, 'model_loaded': bool(M.get('model'))}), mimetype='application/json')

@app.route('/')
def index(): return send_from_directory(WEBUI_DIR, 'web_demo.html')
@app.route('/call')
def call_page(): return send_from_directory(WEBUI_DIR, 'web_demo.html')
...
def init_model(args):
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        M['asr'] = AutoModel(model=os.path.join(MODEL_DIR, 'SenseVoiceSmall'), trust_remote_code=True, device=args.device, disable_update=True)
    ...
    try:
        ...
        M['mimi'] = MimiModel.from_pretrained(os.path.join(MODEL_DIR, 'mimi')).eval().to(args.device)
    ...
    try:
        ...
        sd = torch.load(os.path.join(MODEL_DIR, 'campplus', 'campplus_cn_common.pt'), map_location='cpu')
...
if __name__ == '__main__':
    ...
    p.add_argument('--load_from', default=os.path.join(ROOT_DIR, 'runtime-models'), help='模型权重搜索目录；目录下可放多个 HF 格式模型，WebUI 会自动扫描并允许切换。')
